...
function buildMetadata(schema: JSONSchema7Definition, opts: Options): string[] {
	...
	opts.renderTitle &&
		schema.title &&
		out.push(
			`${"#".repeat(Math.min(opts.headingLevel, 6))} ${schema.title}`,
		);
 ...
 * Builds: if, then, else
 */
function buildConditionalSubschemas(
	schema: JSONSchema7Definition,
	opts: Options,
): string[] {
	if (typeof schema !== "object") {
		...
		if (definitions.length > 1) {
			...
			definitions.forEach((definition) => {
				if (definition[0].startsWith("`object`")) {
					...
					list.push(
						`- ${definition.map((value) => value.replaceAll("\n", " ")).join(" ")}`,
					);
				}
			});
			...
			if (additionalDefinitions.length > 0) {
				...
				additionalDefinitions.forEach((definition, index) => {
					// Render heading
					out.push(
						`${"#".repeat(Math.min(6, opts.headingLevel + 1))} Subdefinition ${index + 1}`,
					);
